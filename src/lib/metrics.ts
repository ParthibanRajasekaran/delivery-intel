...
import { fetchWorkflowRuns, fetchDeployments, fetchMergedPullRequests } from "./github";

// ---------------------------------------------------------------------------
// Rating thresholds (based on DORA State of DevOps benchmarks)
// ---------------------------------------------------------------------------

function rateDeploymentFrequency(perWeek: number): DORAMetrics["deploymentFrequency"]["rating"] {
  if (perWeek >= 7) {
    return "Elite";
  } // multiple per day
  if (perWeek >= 1) {
...
function rateLeadTime(medianHours: number): DORAMetrics["leadTimeForChanges"]["rating"] {
  if (medianHours < 24) {
    return "Elite";
  }
  if (medianHours < 168) {
...
function rateChangeFailureRate(pct: number): DORAMetrics["changeFailureRate"]["rating"] {
  if (pct <= 5) {
    return "Elite";
  }
  if (pct <= 10) {
...
function rateMTTR(hours: number | null): DORAMetrics["meanTimeToRestore"]["rating"] {
  if (hours === null) {
    return "N/A";
  }
  if (hours < 1) {
...
async function computeLeadTime(id: RepoIdentifier): Promise<DORAMetrics["leadTimeForChanges"]> {
...
async function _computeMTTR(id: RepoIdentifier): Promise<DORAMetrics["meanTimeToRestore"]> {
  ...
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].status === "completed" && sorted[i].conclusion === "failure") {
